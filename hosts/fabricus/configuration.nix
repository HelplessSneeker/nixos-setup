{ config, pkgs, lib, inputs, pkgsUnstable, ... }:
{
  ...
  users.users.bfn = {
    ...
    # KEIN eigener SSH-Zugang fuer bfn auf dieser Maschine.
    #
    # Hier stand bis zum 12.08.2026 der Pubkey `openclaw-lab`. Ein privater
    # Teil davon hat nie existiert (nicht im 1Password-Agent, kein einziger
    # Login im sshd-Journal), der Eintrag war also wirkungslos. Entfernt,
    # damit die Config nicht einen Zugang behauptet, den es nicht gibt.
    #
    # Wer wieder rein will, muss hier einen Key eintragen: seit der Haertung
    # vom 07.08.2026 liest sshd nur noch /etc/ssh/authorized_keys.d/%u
    # (modules/ssh-hardening.nix) und Passwort-Auth ist aus -- ein von Hand
    # nach ~/.ssh/authorized_keys gelegter Key zieht NICHT. Deklarativ ist
    # der einzige Weg. Ein PUBLIC key gehoert nicht zu den Secrets und darf
    # im Repo stehen. Passendes Gegenstueck in home/ssh.nix, siehe Kommentar
    # dort. Der Agent-User `skitarii` ist davon unberuehrt, der haengt an
