...
in
{
  ...
  #
  # Tailscale SSH aendert den Host-Key NICHT: tailscaled uebernimmt die
  # vorhandenen /etc/ssh/ssh_host_*_key des Rechners. Am 16.09.2026 an
  # cogitator direkt gemessen -- vor dem Umschalten (sshd, per ssh-keyscan auf
  # 192.168.0.233) und danach (Banner `SSH-2.0-Tailscale` aus dem Tailnet)
  # derselbe ed25519-Key. Nur das Banner wechselt, der Pin bleibt gueltig.
  # Deshalb steht cogitator hier mit Tailnet-Namen UND LAN-IP in einer Zeile.
